...
export default function ReceiptScanner() {
  ...
  const detectCategory = (text) => {
    ...
    const foodKeywords = ["kfc", "mcdonald", "burger king", "pizza hut", "dominos", "zomato", "swiggy", "restaurant", "cafe", "food", "burger", "pizza", "biryani"];
    const fashionKeywords = ["zudio", "trends", "max", "pantaloons", "westside", "h&m", "fashion", "clothing", "shirt", "jeans"];
    const groceryKeywords = ["dmart", "reliance fresh", "supermarket", "grocery", "mart"];
    const medicalKeywords = ["apollo", "pharmacy", "medical", "hospital", "clinic", "medplus"];
    const travelKeywords = ["uber", "ola", "rapido", "petrol", "fuel", "travel"];
    const electronicsKeywords = ["electronics", "mobile", "laptop", "iphone", "samsung", "computer"];

    if (foodKeywords.some((word) => data.includes(word))) return "Food";
    if (fashionKeywords.some((word) => data.includes(word))) return "Fashion";
    if (groceryKeywords.some((word) => data.includes(word))) return "Groceries";
    if (medicalKeywords.some((word) => data.includes(word))) return "Medical";
    if (travelKeywords.some((word) => data.includes(word))) return "Travel";
    if (electronicsKeywords.some((word) => data.includes(word))) return "Electronics";
    return "Others";
  };
  ...
  const extractAmount = (text) => {
    ...
    for (let pattern of priorityPatterns) {
      ...
      if (match) {
        ...
        if (value > 10 && value < 100000) return value.toFixed(2);
      }
    }

    const decimalMatches = cleanText.match(/\d+\.\d{2}/g);
    if (decimalMatches) {
      const validAmounts = decimalMatches.map(Number).filter((num) => num > 10 && num < 100000);
      if (validAmounts.length > 0) return Math.max(...validAmounts).toFixed(2);
    }
  ...
  const extractMerchant = (text) => {
    ...
    for (let line of lines.slice(0, 5)) {
      if (line.length > 2 && !line.match(/\d{5,}/)) {
  ...
  const saveReceipt = async () => {
    try {
      if (!receiptData.merchant || !receiptData.amount || Number(receiptData.amount) <= 0) {
        setMessage("Invalid receipt data");
        return;
      }
      ...
      await axios.post(
        `${BASE_URL}/transaction/input`, // ✅ no more hardcoded localhost
        {
          transactions: [
            {
              type: "expense",
        ...
        {
          headers: { Authorization: `Bearer ${token}` }, // ✅ auth header instead of cookie
        }
      );

      setMessage("Receipt Added Successfully");
      setReceiptData({ merchant: "", amount: "", category: "" });
      setImage(null);
      ...
      <div className="relative overflow-hidden bg-black/60 backdrop-blur-2xl border border-red-500/20 rounded-[36px] p-8 md:p-10 w-full max-w-5xl mx-auto shadow-[0_0_80px_rgba(255,40,0,0.25)]">
        ...
        <div className="relative z-10">
          ...
          <label className="w-full flex flex-col items-center justify-center border-2 border-dashed border-red-500/20 bg-black/40 rounded-3xl p-10 cursor-pointer hover:border-red-400 transition-all duration-300 mb-8">
            ...
            <h2 className="text-2xl font-bold text-white mb-2">Upload Receipt</h2>
            <p className="text-gray-400">JPG, PNG supported</p>
          ...
          {image && (
            <div className="mb-8">
              <img src={image} alt="receipt" className="w-full max-w-md rounded-3xl border border-red-500/20 shadow-xl" />
            </div>
          )}
          ...
          {loading && (
            <div className="mb-6 bg-yellow-500/10 border border-yellow-500/20 rounded-2xl p-4">
              <p className="text-yellow-300 text-lg font-semibold">Scanning Receipt...</p>
            </div>
          )}
          ...
          {message && (
            <div className="mb-6 bg-blue-500/10 border border-blue-500/20 rounded-2xl p-4">
              <p className="text-blue-300 text-lg font-semibold">{message}</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-black/50 border border-red-500/20 rounded-3xl p-6">
              <p className="text-red-200 text-sm mb-3">Merchant</p>
              <h3 className="text-2xl font-bold text-white break-words">{receiptData.merchant || "Not Detected"}</h3>
            </div>
            <div className="bg-black/50 border border-red-500/20 rounded-3xl p-6">
              <p className="text-red-200 text-sm mb-3">Amount</p>
              <h3 className="text-2xl font-bold text-green-400">{receiptData.amount ? `₹${receiptData.amount}` : "Not Detected"}</h3>
            </div>
            <div className="bg-black/50 border border-red-500/20 rounded-3xl p-6">
              <p className="text-red-200 text-sm mb-3">Category</p>
              <h3 className="text-2xl font-bold text-orange-300">{receiptData.category || "Not Detected"}</h3>
            </div>
          </div>
