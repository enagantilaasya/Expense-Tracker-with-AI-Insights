  ...
  const getData = async () => {
    try {
      ...
      const res = await axios.get(
        `${BASE_URL}/transaction/all-transactions`,
        {
        ...
        <div className="relative z-10">
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-8">
            <div className="bg-black/70 border border-red-500/20 rounded-3xl p-6">
              <p className="text-red-300 text-sm uppercase tracking-widest mb-3">Total Expenses</p>
              <h2 className="text-4xl font-bold text-red-400">₹{totalExpense.toFixed(2)}</h2>
            </div>
            <div className="bg-black/70 border border-emerald-500/20 rounded-3xl p-6">
              <p className="text-emerald-300 text-sm uppercase tracking-widest mb-3">Total Income</p>
              <h2 className="text-4xl font-bold text-emerald-400">₹{totalIncome.toFixed(2)}</h2>
            </div>
            <div className="bg-black/70 border border-orange-500/20 rounded-3xl p-6">
              <p className="text-orange-300 text-sm uppercase tracking-widest mb-3">Transactions</p>
              <h2 className="text-4xl font-bold text-orange-400">{filtered.length}</h2>
            </div>
          </div>
          ...
          <div className="bg-black/70 border border-red-500/20 rounded-3xl p-6 overflow-hidden">
            ...
            {loading ? (
              <div className="text-center py-12 text-gray-300 text-xl">Loading Transactions...</div>
            ) : filtered.length === 0 ? (
              <div className="text-center py-12 text-gray-500 text-xl">No Transactions Found</div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full min-w-[900px]">
                  <thead>
                    <tr className="border-b border-red-500/20 text-left">
                      <th className="pb-5 text-red-200">Type</th>
                      <th className="pb-5 text-red-200">Category</th>
                      <th className="pb-5 text-red-200">Amount</th>
                      <th className="pb-5 text-red-200">Payment</th>
                      <th className="pb-5 text-red-200">Description</th>
                      <th className="pb-5 text-red-200">Date</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filtered.map((t, i) => (
                      <tr key={i} className="border-b border-red-500/10 hover:bg-red-500/5 transition-all">
                        <td className="py-5">
                        ...
                        <td className="capitalize text-gray-200">{t.category}</td>
                        <td className={`font-bold text-lg ${t.type === "Income" ? "text-emerald-400" : "text-red-400"}`}>
                          ₹{Number(t.amount).toFixed(2)}
                        </td>
                        <td className="capitalize text-gray-300">{t.payment}</td>
                        <td className="text-gray-400">{t.description}</td>
                        <td className="text-gray-400">{new Date(t.date).toLocaleDateString("en-IN")}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
