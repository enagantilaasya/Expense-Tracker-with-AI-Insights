...
function AIInsights() {
  ...
  const savingsRatio =
    insights.totalIncome > 0
      ? ((insights.savings / insights.totalIncome) * 100).toFixed(1)
      : 0;

  const expenseRatio =
    insights.totalIncome > 0
      ? ((insights.totalExpense / insights.totalIncome) * 100).toFixed(1)
      : 0;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-4xl font-bold text-white">AI analyzing finances...</h2>
          <p className="text-gray-400 mt-3 text-lg">Generating smart financial intelligence</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
          ...
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <h2 className="text-4xl font-extrabold text-emerald-400">
                ₹{Number(insights.totalIncome).toFixed(0)}
              </h2>
            </div>
          ...
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <h2 className="text-4xl font-extrabold text-red-400">
                ₹{Number(insights.totalExpense).toFixed(0)}
              </h2>
            </div>
        ...
        <div className="bg-black/50 border border-cyan-500/20 rounded-3xl p-6 shadow-[0_0_40px_rgba(0,220,255,0.12)]">
          <div className="flex items-center justify-between mb-5">
            <div>
              <p className="text-gray-400 mb-2">Net Savings</p>
              <h2 className={`text-4xl font-extrabold ${insights.savings >= 0 ? "text-cyan-400" : "text-red-400"}`}>
                ₹{Number(insights.savings).toFixed(0)}
              </h2>
            </div>
        ...
        <div className="relative z-10 flex items-start gap-5">
          ...
          <div>
            ...
            <h2 className="text-3xl font-bold text-white leading-relaxed">
              {insights.alert || "No alerts generated yet."}
            </h2>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-black/50 border border-cyan-500/20 rounded-[36px] p-8 shadow-[0_0_40px_rgba(0,220,255,0.12)]">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <p className="text-cyan-300 uppercase tracking-widest text-xs">AI Analysis</p>
              <h3 className="text-3xl font-bold text-white">Financial Health</h3>
            </div>
          </div>
          <div className="space-y-6">
            ...
            <div className="bg-black/40 border border-cyan-500/10 rounded-3xl p-6">
              <div className="flex justify-between items-center mb-4">
                <div>
                  <p className="text-gray-400 mb-2">Savings Ratio</p>
                  <h2 className="text-4xl font-extrabold text-cyan-400">{savingsRatio}%</h2>
                </div>
              </div>
              <div className="w-full h-4 bg-black/60 rounded-full overflow-hidden border border-white/5">
                <div
                  className="h-full rounded-full bg-gradient-to-r from-cyan-500 to-blue-500 transition-all duration-500"
                  style={{ width: `${Math.min(Number(savingsRatio), 100)}%` }}
                />
              </div>
            </div>
            ...
            <div className="bg-black/40 border border-red-500/10 rounded-3xl p-6">
              <div className="flex justify-between items-center mb-4">
                <div>
                  <p className="text-gray-400 mb-2">Expense Ratio</p>
                  <h2 className="text-4xl font-extrabold text-red-400">{expenseRatio}%</h2>
                </div>
              </div>
              <div className="w-full h-4 bg-black/60 rounded-full overflow-hidden border border-white/5">
                <div
        ...
        <div className="bg-black/50 border border-purple-500/20 rounded-[36px] p-8 shadow-[0_0_40px_rgba(168,85,247,0.12)]">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <p className="text-pink-300 uppercase tracking-widest text-xs">Recommendations</p>
              <h3 className="text-3xl font-bold text-white">Smart Suggestions</h3>
            </div>
          </div>
          <div className="space-y-5">
            <div className="bg-black/40 border border-cyan-500/20 rounded-3xl p-6 hover:translate-y-[-2px] transition-all duration-300">
              <p className="text-cyan-300 font-semibold text-lg">💡 Track recurring subscriptions monthly</p>
            </div>
            <div className="bg-black/40 border border-purple-500/20 rounded-3xl p-6 hover:translate-y-[-2px] transition-all duration-300">
              <p className="text-purple-300 font-semibold text-lg">📈 Reduce impulse spending to improve savings</p>
            </div>
            <div className="bg-black/40 border border-emerald-500/20 rounded-3xl p-6 hover:translate-y-[-2px] transition-all duration-300">
              <p className="text-emerald-300 font-semibold text-lg">🎯 Create monthly category-wise budget goals</p>
            </div>
          </div>
        </div>
